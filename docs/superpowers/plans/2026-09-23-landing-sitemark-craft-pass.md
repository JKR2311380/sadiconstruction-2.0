...
> **Status (2026-09-24):** The Site Mark scaffold and phases spec this plan upgrades were never committed, so the branch still carried the formwork landing. Execution rebuilds the six sections from scratch at the craft bar below (TSX, under `src/features/landing/sitemark/`), with this plan as the binding spec. The formwork files stay in the tree, unmounted.
